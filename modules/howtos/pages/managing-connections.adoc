...
[abstract]
{description}
It contains best practices as well as information on TLS/SSL, advanced connection options, and troubleshooting Cloud connections.
...
The simplest way to create a `Cluster` object is to call `cbcolumnar.NewCluster()` with a <<connection-strings, connection string>>, username, and password:

[source,golang,indent=0]
----
include::devguide:example$connecting.go[tag=connecting]
----
