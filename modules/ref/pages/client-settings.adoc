...
[source,go]
----
include::example$configuration.go[tags=configuration]
----
...
[source]
----
couchbases://example.com?timeout.connect_timeout=30s&timeout.server_query_timeout=2m
----
...
|[[timeout.server_query_timeout]]`timeout.server_query_timeout`
|`10m` (10 minutes)
|Default query execution time limit sent to the server. Can be overridden for specific queries by setting the `ServerQueryTimeout` query option when executing the query.

Note: Client side timeouts are controlled by use of `context.Context`.
|===
...
|Empty string / empty list (SDK uses any cipher suite supported by the Go runtime)
...
Consult your Go reference documentation for a list of supported cipher suite names.
